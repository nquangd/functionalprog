...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    ...
    means.foldLeft(groups){ (groups:GenMap[Point, GenSeq[Point]], p:Point)
    => if(groups.contains(p)) groups else groups ++ GenMap[Point, GenSeq[Point]]((p, GenSeq()))}
  ...
  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    val mean = oldMeans.zip(newMeans).foldLeft(0d)((acc:Double, pair) => acc + pair._1.squareDistance(pair._2))
    mean < eta
  }
