...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType =
    StructType(StructField(columnNames.head, StringType, nullable = false) ::columnNames.tail.map(fieldname => StructField(fieldname, DoubleType, nullable = false)))
  ...
  def row(line: List[String]): Row =
    ...
    //Row(line.head::line.tail.map(e => Row(e.trim.toDouble)))
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    val columnwithIndex = columnNames flatMap { str => cat
      .flatMap(ls =>
        if (ls._1.exists(str.startsWith)) Some((ls._2,new Column(str))) else None)
      .sortBy(_._1)
      .headOption  match {
      case None => None
    ...
    val mapping = columnwithIndex.groupBy(x => x._1).mapValues(_.map(_._2))

    val mapped = (0 to 2).map(index => mapping.getOrElse(index, List.empty[Column]))
  ...
  //val tt = Encoders.product[TimeUsageRow].schema
}
