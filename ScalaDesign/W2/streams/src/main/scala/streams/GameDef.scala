...
trait GameDef {
  ...
  case class Block(b1: Pos, b2: Pos) {
    ...
    def neighbors: List[(Block, Move)] = {
      // if standing, four possible moves => four possible neighbours
      // if laying, four possible moves => four possible neighbours
    ...
    def legalNeighbors: List[(Block, Move)] = {
      this.neighbors filter{ case (x,y) => x.isLegal}
    }
