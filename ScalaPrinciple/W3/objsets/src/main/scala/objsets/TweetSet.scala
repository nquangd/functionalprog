...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    def maxrt(tw1:Tweet, tw2:Tweet):Tweet = if (tw1.retweets > tw2.retweets) tw1 else tw2
    if (left.isEmpty && right.isEmpty) elem
    else if (left.isEmpty) maxrt(right.mostRetweeted,elem)
    else if (right.isEmpty) maxrt(left.mostRetweeted,elem)
    else maxrt(left.mostRetweeted,maxrt(right.mostRetweeted,elem))
  }
  /**
   * The following methods are already implemented
   */
